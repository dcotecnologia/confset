...
Then edit `app.rb` and register `Config`
...
Afterwards in need to register `Config` in your app and give it a root so
...
raise a `Config::Validation::Error` containing information about all
...
`config/initializers/config.rb` file:
...
they were environment variables by using `Config::Sources::EnvSource`.
...
This project exists thanks to all the people who contribute and you are very
warmly welcome to help. Please follow our [contribution guidelines](CONTRIBUTING.md).
